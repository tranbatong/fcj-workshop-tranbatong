...
```
#### 2. Modal Chỉnh sửa Hóa đơn (src/components/InvoiceEditModal.jsx)
Một tính năng UX/UI cực kỳ cao cấp: Hiển thị song song (side-by-side) tài liệu gốc (ảnh/PDF) và kết quả bóc tách của AI. Người dùng có thể đối chiếu trực quan và sửa lỗi nếu AI nhận diện sai trước khi lưu lại.
```jsx
import React, { useState, useEffect } from 'react';
